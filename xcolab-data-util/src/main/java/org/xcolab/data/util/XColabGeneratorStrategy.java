...
public class XColabGeneratorStrategy extends DefaultGeneratorStrategy {
    ...
    private static final String[] TABLE_PREFIXES = { "xcolab_", "members_", "contest_", "content_", "proposal_",
            "comment_", "files_", "balloon_", "admin_", "activities_", "flagging_", "filtering_", "sharedcolab_" };
    ...
    public String getJavaIdentifier(Definition definition) {
        ...
        final String underscoreSeparatedName = StringUtils
                .join(unprefixedTableName.split("(?<=[a-z0-9])(?=[A-Z0-9])|(?<=[A-Z0-9])(?=[A-Z0-9][a-z0-9])"), "_");
    ...
    public String getJavaClassName(Definition definition, Mode mode) {
        ...
        if (containsPrefix(outputName)) {
            result.append(cleanUpSchemeAndPrefix(outputName));
            if (mode == Mode.DEFAULT) {
                result.append("Table");
            }
        } else {
            if (outputName.contains("_")) {
